...
- [ ] Resolver el punto abierto de hosting: SQLite (archivo local) no corre en Cloudflare Workers — decidir entre Cloudflare D1 o alojar el backend fuera de Cloudflare (ver [01-Arquitectura §3.5](../01-Arquitectura/README.md))
- [ ] Deploy real: frontend en Cloudflare Pages, backend según lo decidido arriba
- **E2E gate**: checklist de seguridad 100% verificado contra el código corriendo, no de memoria; sitio accesible públicamente y probado de punta a punta ya en producción.
